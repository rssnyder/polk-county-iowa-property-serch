#!/usr/bin/env python3
"""
Load agricultural property data from CSV into SQLite.
Supports incremental updates via INSERT OR REPLACE on dp (parcel ID).

Usage: python3 load_agricultural_properties.py [csv_file] [db_file]
  Defaults: raw/agr/POLKCOUNTY_AGR_*.csv -> polk_county.db
"""
import csv
import sqlite3
import glob
...
# Default paths
CSV_PATTERN = 'raw/agr/POLKCOUNTY_AGR_[0-9]*.csv'  # Matches date pattern, not SALES
...
# Column definitions (from CSV header)
text_cols = [
...
int_cols = [
    'land_full', 'bldg_full', 'agbd_full', 'total_full', 'land_adj', 'bldg_adj', 'agbd_adj', 'total_adj',
...
cursor.execute('CREATE TABLE IF NOT EXISTS agricultural_properties (' + ', '.join(col_defs) + ')')
cursor.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_agr_prop_dp ON agricultural_properties(dp)')
...
with open(csv_file, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        cursor.execute(f'INSERT OR REPLACE INTO agricultural_properties ({col_names}) VALUES ({placeholders})', values)
        count += 1

conn.commit()
final_count = cursor.execute('SELECT COUNT(*) FROM agricultural_properties').fetchone()[0]
print(f"Loaded {count} records from CSV")
print(f"Total agricultural properties in database: {final_count}")
conn.close()
